...
const TransactionList: React.FC<TransactionListProps> = ({ 
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-md sm:shadow-xl border border-gray-100 dark:border-gray-700 transition-all">
      {/* Header */}
      <div className="p-4 sm:p-8 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-700 rounded-t-xl sm:rounded-t-2xl">
        <h2 className="text-base sm:text-2xl font-bold text-gray-800 dark:text-white mb-2 sm:mb-3">Daftar Transaksi</h2>
        <div className="flex items-center gap-3 sm:gap-6 text-xs sm:text-sm flex-wrap">
          <div className="flex items-center space-x-1.5">
            ...
            <span className="text-gray-600 dark:text-gray-300">{formatCurrency(totalIncome)}</span>
          </div>
          <div className="flex items-center space-x-1.5">
            <div className="w-2 h-2 sm:w-3 sm:h-3 bg-red-500 rounded-full"></div>
            <span className="text-gray-600 dark:text-gray-300">{formatCurrency(totalExpenses)}</span>
          </div>
          <span className="text-gray-400 dark:text-gray-500">{filteredTransactions.length} transaksi</span>
        </div>
      </div>

      {/* Filters */}
      <div className="p-3 sm:p-6 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
        {/* Search */}
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
          />
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-4 sm:overflow-visible">
          <select
            ...
            className="flex-shrink-0 px-3 py-2 text-xs sm:text-sm border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
          >
          ...
          <div className="flex space-x-2">
            ...
            <button
              ...
              className="flex-shrink-0 px-2.5 py-2 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors bg-white dark:bg-gray-800"
            >
              {sortOrder === 'asc' ? <SortAsc className="h-4 w-4 text-gray-600 dark:text-gray-300" /> : <SortDesc className="h-4 w-4 text-gray-600 dark:text-gray-300" />}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-700">
        {filteredTransactions.length === 0 ? (
          <div className="p-8 sm:p-12 text-center text-gray-500">
            <Filter className="h-10 w-10 sm:h-16 sm:w-16 mx-auto mb-3 text-gray-300 dark:text-gray-600" />
            <h3 className="text-base sm:text-lg font-medium text-gray-800 dark:text-white mb-1">Tidak ada transaksi</h3>
            <p>Tidak ada transaksi yang sesuai dengan filter yang dipilih</p>
          </div>
        ) : (
          filteredTransactions.map((transaction) => (
            <div key={transaction.id} className="p-3 sm:p-6 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Desktop meta (extra detail) */}
                    <div className="hidden sm:flex items-center gap-4 mt-2 text-sm text-gray-600 dark:text-gray-300">
                ...
                <div className="flex space-x-1 ml-2 sm:opacity-0 sm:group-hover:opacity-100 sm:transition-opacity flex-shrink-0">
                  <button
